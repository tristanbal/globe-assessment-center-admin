...
<div class="container">
    ...
    {{  Form::open(array('action' =>[ 'GroupsPerGapAnalysisSettingRoleController@update',$groupsPerGapAnalysisSettingRole->id]))}}
    <div class="row">
        <div class="col-md-12">
            <div class="row">
                            ...
                            <option value = "" disabled selected>Select a Group Gap Analysis Setting</option>
                                ...
                                @foreach($groupsPerGapAnalysisSetting as $row)
                                    <option data-group = '{{$row->id}}' value = "{{$row->id}}"
                                        @if ($row->id == $groupsPerGapAnalysisSettingRole->gpgas_id_foreign)
                                            selected
                    ...
                    <select id="roleDropdown" class="form-control group" name="roleDropdown" style="width:100%;" >
                        @if(count($role) > 0)
                            <option value = "" disabled selected>Select a Role</option>
                                ...
                                @foreach($role as $row)
                                    <option data-group = '{{$row->id}}' value = "{{$row->id}}"
                                        @if ($row->id == $groupsPerGapAnalysisSettingRole->roleID)
                                        selected
                                        @endif
                                        >{{$row->name}}</option>
                                @endforeach
