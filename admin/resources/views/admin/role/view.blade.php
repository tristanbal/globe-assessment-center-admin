    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="row">
                ...
                <div class="col-sm-9">
                    <h3 class="">Employees Under {{$role->name}}</h3>
                    <hr>
                            ...
                            <div class="card-body pb-0">
                                @foreach ($modelCompetencies as $item)
                                    @foreach ($competency as $comp)
                                        @if ($item->competencyID == $comp->id && $item->competencyTypeID == $types->id)
                                            <div class="d-flex">
                                                <div class="avatar">
                                                    <img src="{{asset('img/logoproduct.svg')}}" alt="..." class="avatar-img rounded-circle">
                                                </div>
                                                <div class="flex-1 pt-1 ml-2">
                                                    <h6 class="fw-bold mb-1">{{$comp->name}}</h6>
                                                    <small class="text-muted">{!! nl2br($comp->definition) !!}</small>
                                                </div>
                                                <div class="d-flex ml-auto align-items-center">
                                                    <h3 class="text-info text-justify fw-bold">L{{$item->targetLevelID - 1}}</h3>
                                                </div>
                                            </div>
